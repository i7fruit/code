...
int main(int argc, string argv[])
{
    ...
    for (int i = 0, j = 0, n = strlen(p_txt), m = strlen(k_word); i < n; i++, j++)
    {
        /**
         ...
         * }
         *
         * or, simply do j % m so the keyword index wraps around
         */
        if (isalpha(p_txt[i]))
        {
            c_txt[i] = (alpha_index(p_txt[i]) + alpha_index(k_word[j % m])) % 26;
