...
int main(int argc, char *argv[])
{
    ...
    // for (int i = 0, infile_Height = abs(bi_copy.biHeight); i < infile_Height; i++)
    // {
    //     // Resizes vertically
    //     for (int m = 0; m < n; m++)
    //     {
    //         // Iterates over pixels in scanline
    //         for (int j = 0; j < bi_copy.biWidth; j++)
    //         {
    //             // Temporary storage
    //             RGBTRIPLE triple;
    ...
    //             // Writes RGB triple to outfile n times
    //             for (int k = 0; k < n; k++)
    //             {
    ...
    //         if (m + 1 != n)
    //         {
    //             fseek(inptr, -((sizeof(RGBTRIPLE) * bi_copy.biWidth) + infile_padding), SEEK_CUR);
    //         }
    //     }
    // }
